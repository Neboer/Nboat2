...
router.delete('/:blog_hex_id/:article_index', validator.params(schema.one_blog_id_with_article_index), (req, res, next) => {
    if (req.params.article_index) { // 传递article_index一定是删去大博文中的小文章，不传一定是删去整个博文，无论大小。
        return database.delete_article_from_big_blog(req.collection, req.params.blog_hex_id, req.params.article_index)
    } else {
        return database.delete_blog_by_id(req.collection, req.params.blog_hex_id)
    }
})
